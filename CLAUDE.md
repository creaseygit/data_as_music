...
5. `server.py` `broadcast_loop` — Per tick: samples mid once per watched market via `scorer.sample_mid()`, then per-client normalizes to 0–1, applies the unified sensitivity slider (scales the `price_delta_band` thresholds, the lookback window, the event firing thresholds, and the power-curve exponent for `heat`/`trade_rate`/`spread` together), detects events with magnitudes, emits the server-decided `price_delta_band` (-3..+3) alongside raw cents, pushes JSON via WebSocket every 3s
...
| `frontend/tracks/*.js`    | Track files (auto-discovered, dynamically loaded): `late_night_in_bb.js` (Late Night in Bb — 8-voice jazz trio with voice gain system), `digging_in_the_markets.js` (Digging in the Markets — 8-voice lo-fi hip hop with swung drums and Rhodes), `weather_vane.js` (Weather Vane — single-voice vibraphone price-direction alert in the `alert` category), `so_over_so_back.js` (So Over, So Back — meme-voice sampler in the `funny` category; six-level signed intensity scale from `so_fucking_over` (big down) through `over` / `back` to `so_fucking_back` (big up), with firing density scaling alongside sample intensity), `diagnostics.js` (Diagnostics — silent track in the `diagnostic` category that uses the browser `SpeechSynthesis` API to speak market switches and price changes. Announces market title on rotation, the new price percent on every ≥0.5¢ change, and falls back to a sensitivity-scaled "Data OK. Price X." heartbeat when the market is flat. Utterances are throttled to ≥5s apart and skip if speech is already in flight, so rapid price ticks don't queue up; picks a UK-female voice when available), `_template.js` (annotated starter template for new tracks with voice/mastering support). Drop a new `.js` file here and restart the server — no other changes needed |
| `samples/`                | Local audio samples served at `/samples/` by the aiohttp static route. Registered as Strudel aliases in `audio-engine.js` prebake (`samples({ so_over: '/samples/so_over.mp3', ... })`) and referenced as `.s("so_over")` in tracks. See the `dam-strudel` skill for full sample workflow. |
...
| --- | -------- | ------------ |
| [`docs/musician-brief.md`](docs/musician-brief.md) | **Give this to musicians.** No-code brief: what each signal means musically, the four market moods, signal combinations, events, sensitivity, quick reference card | Briefing musicians, onboarding collaborators, explaining the data-to-music mapping |
| [`docs/data-interface.md`](docs/data-interface.md) | Technical data interface: all values pushed to clients (incl. `price_delta_band`), the unified sensitivity slider (band thresholds + window + power-curve + event thresholds scaled together), event triggers with magnitudes, momentum/volatility computation details | Working on data pipeline, scorer, mixer, or understanding signal internals |
...
| [`docs/development/mastering-and-sandbox.md`](docs/development/mastering-and-sandbox.md) | **Implemented.** Design spec for mastering page (per-voice gain mixing), sandbox page (data simulation), voice gain system, JSON export format. Both pages live at `/master` and `/sandbox`. All music tracks (`late_night_in_bb`, `digging_in_the_markets`) are migrated to the voice gain system | Modifying mastering/sandbox pages, adding voices to new tracks, understanding the gain multiplier flow |