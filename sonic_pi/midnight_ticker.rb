...
live_loop :kick do
  h = get(:heat)
  amp_val = 0.15 + (h * 0.55)
  sample :bd_haus, amp: amp_val * 0.88, cutoff: 80  # ~nf
  # Double-hit on high activity
  if h > 0.7 && spread(3,8).look
    sleep 0.25
    sample :bd_haus, amp: amp_val * 0.6 * 0.88, cutoff: 70  # ~nf
    sleep 0.75
  else
    sleep 1
  end
...
live_loop :hats do
  ...
  amp_val = 0.05 + (h * 0.15)
  if rand < (0.3 + tr * 0.5)
    sample :drum_cymbal_closed, amp: amp_val * 2.48, rate: 1.5 + rand(0.5), finish: 0.08  # ~nf
  end
  sleep div
end
...
live_loop :snare do
  sleep 1
  h = get(:heat)
  sample :sn_dub, amp: (0.1 + (h * 0.25)) * 0.78, cutoff: 90 + (h * 20), rate: 1.1  # ~nf
  sleep 1
end
...
live_loop :bass do
  ...
  notes.each do |n|
    play n, amp: amp_val * 0.6, release: 0.2, cutoff: cut, res: res_val, wave: 1  # ~nf
    sleep 0.25
  end
end
...
live_loop :sub do
  ...
  h = get(:heat)
  use_synth :sine
  play r - 12, amp: (0.2 + (h * 0.15)) * 0.23, release: 1.8, attack: 0.1  # ~nf
  sleep 2
end
...
live_loop :pad do
  ...
  with_fx :reverb, room: 0.7 + (get(:velocity) * 0.25), mix: 0.6 do
    with_fx :lpf, cutoff: 70 + (pr * 30) do
      play notes, amp: amp_val * 2.66, attack: 1.5, release: 4, pan: rrand(-0.3, 0.3)  # ~nf
    end
  end
  sleep 4
end
  ...
  if tr < 0.3
    ...
    with_fx :reverb, room: 0.5, mix: 0.4 do
      with_fx :echo, phase: 0.375, decay: 3, mix: 0.3 do
        ...
        phrase_len.times do
          n = sc.choose
          play n, amp: amp_val * 0.6, release: 0.15, cutoff: cut  # ~nf
...
live_loop :texture do
  ...
  if v > 0.2
    ...
    with_fx :reverb, room: 0.85, mix: 0.7 do
      play sc.choose, amp: (0.03 + (v * 0.06)) * 1.04, attack: 0.5, release: 2, cutoff: 90  # ~nf
    end
  end
  sleep [3, 4, 5].choose
end
...
live_loop :events do
  ...
  if get(:event_spike) == 1
    set :event_spike, 0
    sample :drum_cymbal_hard, amp: 0.25 * 1.1, rate: 0.8  # ~nf
    sc = pick_scale(3)
    use_synth :blade
    play chord(sc[0], :minor7), amp: 0.15 * 1.04, release: 1.5, cutoff: 95  # ~nf
  end
  # Price move — arpeggio
  ...
  if pm != 0
    ...
    with_fx :echo, phase: 0.25, decay: 2, mix: 0.4 do
      notes.each do |n|
        play n, amp: 0.08 * 0.6, release: 0.1, cutoff: 100  # ~nf
        sleep 0.1
      end
    end
  end
...
live_loop :ambient do
  if get(:ambient_mode) == 1
    ...
    with_fx :reverb, room: 0.9, mix: 0.8 do
      play [:e3, :b3, :e4].choose, amp: 0.15 * 2.66, attack: 3, release: 5  # ~nf
    end
  end
  sleep 6
end
