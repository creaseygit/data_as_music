...
- **Relabel the client-side "Sensitivity" slider as "Time window"** with named presets (Scalper / Intraday / Swing / Event, or explicit durations like 15s / 2m / 15m / 1h) — matches how traders read charts and makes the contract legible. The existing 0–1 value stays as the underlying wire format; this is pure UX. *(Update: the Time-window relabel landed and was later reverted to a bare percentage "Sensitivity" slider once we added server-side band thresholds — exposing the analysis window to the user leaked an implementation detail. The unified-knob model from this section still holds; the UI just doesn't name it as a time window.)*
