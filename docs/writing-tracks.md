...
### weather_vane.js
Melody-only price direction alert, `category: "alert"`, 20 cpm. **Uses evaluate mode.** **1 voice:** `melody` (vibraphone, `gm_vibraphone.n(4)` for the GeneralUserGS variant so high-register runs don't hit the corrupt default-variant zones). Gated on **`price_move`** (edge-detected) — NOT `momentum` — because `momentum` is a lagged MACD-style divergence that hovers above zero on any jittery book, making it useless as a silence gate. `price_move` emits exactly zero when the price is flat, so `|price_move| < 0.05` reliably silences the voice. The sign of `price_move` selects an ascending or descending phrase set, the magnitude selects one of three density bands (sparse pings at 0.05–0.25, 5-note scale runs at 0.25–0.55, 8-note octave-spanning runs with `.sometimes(add(note(12)))` sparkle at 0.55+), and the scale follows `tone` (C4:major when bullish, C4:minor when bearish). Sensitivity integration is automatic — the server applies a power curve to `price_move` after edge detection, so the slider still controls how readily the alert fires. Variation techniques: `<>` alternation between 2–3 phrase variants per band, `.iter(3)` on bands 1–2 to rotate the starting note, and the cycle-deterministic `.sometimes(...)` on band 2. No `onEvent()` ornamentation — the alert stays single-voice by design.
