...
```javascript
{
  ...
  price_moving: bool,         // Per-tick gate — true iff smoothed mid changed ≥0.05¢ this cycle
  price_delta_band: -3..+3,   // Server-decided sensitivity-scaled band (sign=direction, |band|=size, 0=silence)
  price_delta_cents: ±cents,  // Raw cents over the lookback (use for gain saturation, not gating)
  price_move: -1.0-1.0,       // Legacy unitless integrator (same direction as price_delta_band)
...
| `price_moving` + `price_delta_band` | **Phrase trigger + size** — something is happening NOW, this big | Ascending/descending melodic runs, arpeggios, sample selection (canonical melody-from-price recipe) |
| `price_delta_cents` | **Raw move size** — exact cents moved over the lookback | Gain-saturation curves within an active band |
| `price_move` | **Legacy phrase trigger** — same direction as `price_delta_band` but unitless and decaying | Older tracks; new tracks should prefer the band+cents pair |
...
The user's **sensitivity slider** is a single 0–100% reactivity knob applied server-side before your track sees data. You don't need to handle it; just be aware of what it does:

- **Band thresholds** (`price_delta_band`): scale together by `4^(1−2·sens)`. At sens=0.5 they sit at 0.5/2/5¢; at sens=0 they widen to 2/8/20¢; at sens=1 they tighten to 0.125/0.5/1.25¢. Both deadzone and ramps stretch in lockstep — band shape stays the same, only what counts as "small/medium/large" changes.
- **Lookback window** (`price_delta_cents`, `price_move`, `momentum`, `velocity`, `volatility`): 15s at sens=1, ~4min at default, 1hr at sens=0. Wider windows average out short flickers so they don't reach the band thresholds.
- **Activity signals** (`heat`, `trade_rate`, `spread`): power-curve amplitude scaling — high sens inflates small values, low sens crushes them.
- **Event thresholds**: scale by `√(half_life/default)` so small moves fire events at high sens, only big jumps fire at low sens.

If you read `price_delta_band` directly (recommended for melody/voice work), you don't need to think about any of this — the bands stay -3..+3 at every sensitivity, and the slider just decides what the bands *mean*.
...
`meter: 'delta'` renders a 7-cell horizontal stepped gauge (3 down / silence / 3 up) keyed to `price_delta_band` and `price_moving`. The active cell mirrors the server-decided band (sensitivity-scaled — the cells always cover three magnitudes per direction, but what each cell represents in cents depends on the sensitivity slider). The whole gauge dims when `price_moving` is false. Use this for melody/alert voices whose musical decision is "scale length + direction" rather than "loudness".
...
### late_night_in_bb.js
Jazz piano trio with two market paradigms, 120 BPM. **Uses evaluate mode** — strudel code built dynamically with data-driven interpolation. **9 voices:** `comp`, `bass`, `melody`, `ride`, `hihat`, `ghostSnare`, `crossStick`, `kick`, `fill` — all with mastering gain support via `getGain()`. The `melody` voice opts into `meter: 'delta'` for the Now-Playing gauge. **Two paradigms driven by `tone`:** Bullish (tone=1) plays Bb major changes (Cm7→F7→BbΔ7→EbΔ7, ii-V-I-IV) with ascending bass walks. Bearish (tone=0) plays G minor changes (Am7b5→D7→Gm7→Cm7, iiø-V-i-iv) with descending bass walks. **Intensity axis:** `trade_rate` (60%) + `velocity` (40%) compute an intensity score quantized into 3 bands (low/mid/high). Each band selects from pre-composed bass (16 bars × 6 variants) and percussion patterns of increasing complexity. Heat controls overall energy/gain scaling. **Bass and chord cycling follow `momentum`** — the longer-term trend drives the harmonic scaffold. **Melody uses the canonical Weather Vane recipe**: gated on `price_moving`, direction + magnitude band from `price_delta_band` (-3..+3), velocity and gain saturated by raw `price_delta_cents`. Melody is silent when price isn't ticking *or* when the move sits inside the sensitivity-scaled deadzone — the rest of the trio carries the room tone. **Volatility** drives timbral uncertainty (piano detuning, delay feedback, bass LPF darkness). All layers use `.orbit()` for bus isolation.

### digging_in_the_markets.js
Dusty, mellow lo-fi hip hop beats, 80 BPM (cpm 20). **Uses evaluate mode.** **8 voices:** `kick`, `snare`, `hihat`, `keys`, `bass`, `melody`, `texture`, `pad` — all with mastering gain support via `getGain()`. The `melody` voice opts into `meter: 'delta'` for the Now-Playing gauge. **Harmonic system driven by `tone`:** Bullish (tone=1) plays Bb major; bearish (tone=0) plays G minor. Rhodes-style keys comp with data-driven voicings. **Heat controls layer density** (sparse → full kit). **Intensity bands** drive drum complexity (rim shots → swung 8ths → dropout 16ths). **Momentum** drives Rhodes voicings, bass walk direction, and pad progression — the longer-term trend mood. **Melody uses the canonical Weather Vane recipe**: gated on `price_moving`, direction + magnitude from `price_delta_band`, gain saturated by raw `price_delta_cents`. Melody is silent when price isn't ticking — heat alone won't fire it. **Volatility** adds reverb depth, detuning, and wobble. **Price** tints the global filter for warmth. Vinyl texture and warm sine bass throughout.

### so_over_so_back.js
Price-direction meme sampler, `category: "funny"`, 20 cpm. **Uses evaluate mode.** **1 voice:** `voice` (sample bus with `meter: 'delta'`). Canonical Weather Vane recipe: `price_moving` is the gate, `price_delta_band` picks the sample and firing density. Six-level signed intensity scale maps 1:1 onto the delta gauge cells: ▼8 = `so_fucking_over` (band -3), ▼5 = `so_over` (-2), ▼3 = `over` (-1), ▲3 = `back` (+1), ▲5 = `so_back` (+2), ▲8 = `so_fucking_back` (+3). Firing density scales with |band| — small bands fire every ~9s, medium every ~6s, large every cycle. Raw cents shape the gain saturation. No `onEvent()` ornamentation — single-voice by design.

### weather_vane.js
Melody-only price direction alert, `category: "alert"`, 20 cpm. **Uses evaluate mode.** **1 voice:** `melody` (vibraphone, `gm_vibraphone.n(4)` for the GeneralUserGS variant so high-register runs don't hit the corrupt default-variant zones). Opts into `meter: 'delta'` for the Now-Playing gauge. Dual signal use: **`price_moving`** is the gate (true iff price ticked ≥0.05¢ this cycle — false kills the voice immediately) and **`price_delta_band`** is the magnitude (server-decided sensitivity-scaled cell index → scale length: 3-note at |band|=1, 5-note at |band|=2, 8-note at |band|=3). The pairing solves the "rolling delta keeps firing after the move ends" problem — the gate stops the voice the moment the price holds, while the band reflects the size of the move that just happened. Sign of the band picks ascending vs descending phrases; scale follows `tone` (C4:major / C4:minor). Sensitivity widens the band thresholds (and lookback window) together — at low sens only large moves reach band 1+. Phrases are packed into the start of the cycle followed by silence so notes land tightly grouped (~0.2s apart). No `onEvent()` ornamentation — single-voice by design. **The same melody pattern is used by `digging_in_the_markets`, `late_night_in_bb`, and `so_over_so_back` — see those entries for how the pattern slots into a fuller arrangement.**
