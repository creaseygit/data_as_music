...
### late_night_in_bb.js
Jazz piano trio with two market paradigms, 120 BPM. **Uses evaluate mode** — strudel code built dynamically with data-driven interpolation. **9 voices:** `comp`, `bass`, `melody`, `ride`, `hihat`, `ghostSnare`, `crossStick`, `kick`, `fill` — all with mastering gain support via `getGain()`. The `melody` voice opts into `meter: 'delta'` for the Now-Playing gauge. **Two paradigms driven by `tone`:** Bullish (tone=1) plays Bb major changes (Cm7→F7→BbΔ7→EbΔ7, ii-V-I-IV) with ascending bass walks. Bearish (tone=0) plays G minor changes (Am7b5→D7→Gm7→Cm7, iiø-V-i-iv) with descending bass walks. **Intensity axis:** `trade_rate` (60%) + `velocity` (40%) compute an intensity score quantized into 3 bands (low/mid/high). Each band selects from pre-composed bass (16 bars × 6 variants) and percussion patterns of increasing complexity. Low: quarter-note walks, sparse comping, simple hi-hat. Mid: eighth-note approaches, ghost snares, cross-stick. High: chromatic runs, kick bombs, turnaround fills, dense comping. Heat controls overall energy/gain scaling. **Bass and chord cycling follow `momentum`** — the longer-term trend drives the harmonic scaffold. **Melody uses the Weather Vane two-signal pattern**: gated on `price_moving`, direction from sign(`price_delta_cents`), magnitude band picked by |cents| (0.5–2¢ → MOTIF_RISE/FALL_LOW, 2–5¢ → MED, ≥5¢ → HIGH). Melody is silent when price isn't ticking — the rest of the trio carries the room tone. Velocity range and gain saturate with cents (≈10¢ ceiling). **Volatility** drives timbral uncertainty: piano comping detuning (`.speed(rand.range(...))`), delay feedback (more wash/echo), bass LPF (darker tone). All layers use `.orbit()` for bus isolation. Spike events trigger crash cymbal scaled by event magnitude.

### digging_in_the_markets.js
Dusty, mellow lo-fi hip hop beats, 80 BPM (cpm 20). **Uses evaluate mode.** **8 voices:** `kick`, `snare`, `hihat`, `keys`, `bass`, `melody`, `texture`, `pad` — all with mastering gain support via `getGain()`. The `melody` voice opts into `meter: 'delta'` for the Now-Playing gauge. **Harmonic system driven by `tone`:** Bullish (tone=1) plays Bb major; bearish (tone=0) plays G minor — flat keys chosen for that warm lo-fi register. Rhodes-style keys comp with data-driven voicings. **Heat controls layer density:** sparse pulses at low heat, fuller kits and pickups at higher heat. **Intensity bands** (same `trade_rate`/`velocity` formula as `late_night_in_bb`) drive drum complexity: simple rim shots at band 0, swung 8th-note hats at band 1, dropout 16ths at band 2. **Momentum** drives Rhodes voicings, bass walk direction, and pad progression — the longer-term trend mood. **Melody uses the Weather Vane two-signal pattern**: gated on `price_moving`, direction from sign(`price_delta_cents`), magnitude band from |cents| (0.5–2¢ LOW, 2–5¢ MED, ≥5¢ HIGH). Melody is silent when price isn't ticking — heat alone won't fire it. Gain ramps with cents saturation. **Volatility** adds reverb depth, detuning, and wobble. **Price** tints the global filter for warmth. Vinyl texture and warm sine bass throughout. Spike events add crash; price_step events trigger melodic runs.

### so_over_so_back.js
Price-direction meme sampler, `category: "funny"`, 20 cpm. **Uses evaluate mode.** **1 voice:** `voice` (sample bus with `meter: 'delta'`). Same Weather Vane two-signal pattern: `price_moving` is the gate, `price_delta_cents` picks the sample and firing density. Six-level signed intensity scale maps the cents bands 1:1 onto the delta gauge: ▼8 = `so_fucking_over` (≥5¢ down), ▼5 = `so_over` (2–5¢ down), ▼3 = `over` (0.5–2¢ down), ▲3 = `back` (0.5–2¢ up), ▲5 = `so_back` (2–5¢ up), ▲8 = `so_fucking_back` (≥5¢ up). Firing density layers on top — band 0 fires every ~9s, band 1 every ~6s, band 2 every cycle — so big moves feel heavier (wilder sample) and more insistent. Silent when the price isn't ticking. No `onEvent()` ornamentation — single-voice by design.

### weather_vane.js
Melody-only price direction alert, `category: "alert"`, 20 cpm. **Uses evaluate mode.** **1 voice:** `melody` (vibraphone, `gm_vibraphone.n(4)` for the GeneralUserGS variant so high-register runs don't hit the corrupt default-variant zones). Opts into `meter: 'delta'` for the Now-Playing gauge. Dual signal use: **`price_moving`** is the gate (true iff price ticked ≥0.05¢ this cycle — false kills the voice immediately) and **`price_delta_cents`** is the magnitude (rolling cents delta → scale length: 3-note at 0.5–2¢, 5-note at 2–5¢, 8-note at >5¢). This pairing solves the "rolling delta keeps firing after the move ends" problem — the gate stops the voice the moment the price holds, while the cents magnitude still reflects the size of the move that just happened. Sign of the delta picks ascending vs descending phrases; scale follows `tone` (C4:major / C4:minor). Sensitivity scales the server-side lookback (15s at max → 5min at min). Phrases are packed into the start of the cycle followed by silence so notes land tightly grouped (~0.2s apart). No `onEvent()` ornamentation — single-voice by design. **The same melody pattern is used by `digging_in_the_markets`, `late_night_in_bb`, and `so_over_so_back` for their melody/voice — see those entries for how the pattern slots into a fuller arrangement.**
