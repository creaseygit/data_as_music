...
### weather_vane.js
Melody-only price direction alert, `category: "alert"`, 20 cpm. **Uses evaluate mode.** **1 voice:** `melody` (vibraphone, `gm_vibraphone.n(4)` for the GeneralUserGS variant so high-register runs don't hit the corrupt default-variant zones). Driven almost entirely by `momentum`: the sign selects an ascending or descending phrase set, the magnitude selects one of three density bands (sparse pings at 0.05–0.25, 5-note scale runs at 0.25–0.55, 8-note octave-spanning runs with `.sometimes(add(note(12)))` sparkle at 0.55+), and the scale follows `tone` (C4:major when bullish, C4:minor when bearish). When `|momentum| < 0.05` the voice emits `$: silence` — no movement, no sound. Sensitivity doesn't need to be read directly; `momentum` is already sensitivity-scaled server-side via window length, so the slider naturally controls how easily the alert fires. Variation techniques: `<>` alternation between 2–3 phrase variants per band, `.iter(3)` on bands 1–2 to rotate the starting note, and the cycle-deterministic `.sometimes(...)` on band 2. No `onEvent()` ornamentation — the alert stays single-voice by design.
