...
### weather_vane.js
Melody-only price direction alert, `category: "alert"`, 20 cpm. **Uses evaluate mode.** **1 voice:** `melody` (vibraphone, `gm_vibraphone.n(4)` for the GeneralUserGS variant so high-register runs don't hit the corrupt default-variant zones). Driven by **`price_delta_cents`** — the canonical "did the price move" signal in cents. Sign picks ascending vs descending phrases; magnitude in cents picks one of three scale lengths: 3-note run at 0.5–2¢, 5-note run at 2–5¢, 8-note octave run above 5¢. Below 0.5¢ the voice is silent, so flat markets and the warmup window produce no sound. The scale follows `tone` (C4:major when bullish, C4:minor when bearish). Sensitivity scales the lookback window server-side (15s at max → 5min at min), so cranking the slider makes Weather Vane react to shorter-horizon moves. Phrases are packed into the start of the cycle followed by silence so notes land tightly grouped (~0.2s apart) rather than dispersed over the full 3s cycle. No `onEvent()` ornamentation — the alert stays single-voice by design.
